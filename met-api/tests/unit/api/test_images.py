...
from tests.utilities.factory_scenarios import ( TestJwtClaims, TestImageInfo, TestTenantInfo )
from tests.utilities.factory_utils import ( factory_auth_header, factory_tenant_model, set_global_tenant )
...
@pytest.mark.parametrize('image_data', [TestImageInfo.image_missing_unique_name, TestImageInfo.image_missing_display_name, TestImageInfo.image_missing_date_uploaded_name])
...
def test_cannot_get_images_with_different_tenant_ids(client, jwt, session): # pylint:disable=unused-argument
    ...
    # Create tenant 1
    tenant_1 = TestTenantInfo.tenant1
    ...
    # Create tenant 2
    tenant_2 = TestTenantInfo.tenant2
    ...
    # Create a user for tenant 1
    user_1 = copy.deepcopy(TestJwtClaims.met_admin_role.value)
    user_1["tenant_id"] = tenant_1.id

    # Create a user for tenant 2
    user_2 = copy.deepcopy(TestJwtClaims.met_admin_role.value)
    user_2["tenant_id"] = tenant_2.id

    session.commit()

    # Create image for tenant 1
    ...
    assert int(response_tenant_id) == int(user_tenant_id)

    # Assert user 1 can see image
    headers = factory_auth_header(jwt=jwt, claims=user_1)
    headers[TENANT_ID_HEADER] = tenant_1_short_name
    rv = client.get('/api/image_info/', headers=headers, content_type=ContentType.JSON.value)
    assert rv.json.get('total') == 1

    # Assert user from different tenant cannot see image
